...
namespace PluginMaster
{
    public class PWBToolbar : UnityEditor.EditorWindow
    {
        ...
        private void ToolsGUI()
        {
            ...
            {
                var newtool = ToolController.current;

                var floorSelected = newtool == ToolController.Tool.FLOOR;
                newtool = (GUILayout.Toggle(floorSelected, _floorIcon, _btnStyle)
                ? ToolController.Tool.FLOOR : (floorSelected ? ToolController.Tool.NONE : newtool));
                ...
                var pinSelected = newtool == ToolController.Tool.PIN;
                newtool = (GUILayout.Toggle(pinSelected, _pinIcon, _btnStyle)
                ? ToolController.Tool.PIN : (pinSelected ? ToolController.Tool.NONE : newtool));

                var brushSelected = newtool == ToolController.Tool.BRUSH;
                newtool = (GUILayout.Toggle(brushSelected, _brushIcon, _btnStyle)
                ? ToolController.Tool.BRUSH : (brushSelected ? ToolController.Tool.NONE : newtool));

                var gravitySelected = newtool == ToolController.Tool.GRAVITY;
                newtool = (GUILayout.Toggle(gravitySelected, _physicsIcon, _btnStyle)
                ? ToolController.Tool.GRAVITY : (gravitySelected ? ToolController.Tool.NONE : newtool));

                var lineSelected = newtool == ToolController.Tool.LINE;
                newtool = (GUILayout.Toggle(lineSelected, _lineIcon, _btnStyle)
                ? ToolController.Tool.LINE : (lineSelected ? ToolController.Tool.NONE : newtool));

                var shapeSelected = newtool == ToolController.Tool.SHAPE;
                newtool = (GUILayout.Toggle(shapeSelected, _shapeIcon, _btnStyle)
                ? ToolController.Tool.SHAPE : (shapeSelected ? ToolController.Tool.NONE : newtool));

                var tilingSelected = newtool == ToolController.Tool.TILING;
                newtool = (GUILayout.Toggle(tilingSelected, _tilingIcon, _btnStyle)
                ? ToolController.Tool.TILING : (tilingSelected ? ToolController.Tool.NONE : newtool));

                var replaceSelected = newtool == ToolController.Tool.REPLACER;
                newtool = (GUILayout.Toggle(replaceSelected, _replaceIcon, _btnStyle)
                ? ToolController.Tool.REPLACER : (replaceSelected ? ToolController.Tool.NONE : newtool));
                ...
                var selectionSelected = newtool == ToolController.Tool.SELECTION;
                newtool = (GUILayout.Toggle(selectionSelected, _selectionIcon, _buttonWithAxesStyle)
                ? ToolController.Tool.SELECTION : (selectionSelected ? ToolController.Tool.NONE : newtool));
                GUILayout.Space(1);
                ...
                using (new UnityEditor.EditorGUI.DisabledGroupScope(!selectionSelected))
                {
                    using (new GUILayout.HorizontalScope())
                    {
                        ...
                        {
                            SelectionToolController.settings.move = GUILayout.Toggle(SelectionToolController.settings.move,
                                _tIcon, _axisButtonStyle);
                            SelectionToolController.settings.rotate = GUILayout.Toggle(SelectionToolController.settings.rotate,
                                _rIcon, _axisButtonStyle);
                            SelectionToolController.settings.scale = GUILayout.Toggle(SelectionToolController.settings.scale,
                ...
                var circleSelectSelected = newtool == ToolController.Tool.CIRCLE_SELECT;
                newtool = (GUILayout.Toggle(circleSelectSelected, _circleSelectIcon, _btnStyle)
                ? ToolController.Tool.CIRCLE_SELECT : (circleSelectSelected ? ToolController.Tool.NONE : newtool));

                var extrudeSelected = newtool == ToolController.Tool.EXTRUDE;
                newtool = (GUILayout.Toggle(extrudeSelected, _extrudeIcon, _btnStyle)
                ? ToolController.Tool.EXTRUDE : (extrudeSelected ? ToolController.Tool.NONE : newtool));

                var mirrorSelected = newtool == ToolController.Tool.MIRROR;
                newtool = (GUILayout.Toggle(mirrorSelected, _mirrorIcon, _btnStyle)
                ? ToolController.Tool.MIRROR : (mirrorSelected ? ToolController.Tool.NONE : newtool));

                if ((check.changed || _toolChanged) && !TRSChanged)
                {
                    _toolChanged = false;
                    ToolController.current = newtool;
                }
            }
        }
            ...
            using (new UnityEditor.EditorGUI.DisabledGroupScope(!settings.snappingEnabled))
            {
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    else
                    {
                        ...
                        GridManager.settings.snappingOnY = GUILayout.Toggle(settings.snappingOnY,
