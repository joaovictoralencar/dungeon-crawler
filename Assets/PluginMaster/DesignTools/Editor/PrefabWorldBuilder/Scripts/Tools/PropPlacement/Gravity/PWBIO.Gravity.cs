...
namespace PluginMaster
{
    public static partial class PWBIO
    {
        ...
        private static void GravityToolDuringSceneGUI(UnityEditor.SceneView sceneView)
        {
            ...
            if (PWBToolRaycast(mouseRay, out RaycastHit hit, out GameObject c, float.MaxValue, -1,
            ...
            {
                GravityToolController.settings.height += value;
            ...
            {
                var paintedObjectsDic = Paint(GravityToolController.settings, PAINT_CMD, false);
                ...
                var finalPoses = GravityUtils.SimulateGravity(paintedObjects, GravityToolController.settings.simData, false);
            ...
            else if (PWBSettings.shortcuts.gravitySurfDist.Check())
            {
                ...
                GravityToolController.settings.height = Mathf.Max((GravityToolController.settings.height + delta * 0.5f)
        ...
        {
            var settings = GravityToolController.settings;
            ...
            hit.point = SnapAndUpdateGridOrigin(hit.point, GridManager.settings.snappingEnabled,
                paintOnPalettePrefabs: true, paintOnMeshesWithoutCollider: true, ignoresceneColliders: true,
            ...
            {
                DrawSquareIndicator(hit.point, settings.radius, settings.height, tangent, bitangent,
                    normal: Vector3.up, drawDropArea: true);
            }
            else
            {
                DrawCircleIndicator(hit.point, hit.normal,
            ...
            Vector3 bitangent, Camera camera, Transform surface)
        {
            ...
            foreach (var strokeItem in BrushstrokeManager.brushstroke)
            {
                ...
                if (GravityToolController.settings.overwriteBrushProperties)
                    brushSettings = GravityToolController.settings.brushSettings;
                ...
                if (GravityToolController.settings.orientAlongBrushstroke)
                {
                ...
                var layer = GravityToolController.settings.overwritePrefabLayer
                    ? GravityToolController.settings.layer : prefab.layer;
                Transform parentTransform = GetParent(GravityToolController.settings, prefab.name, false, surface);
