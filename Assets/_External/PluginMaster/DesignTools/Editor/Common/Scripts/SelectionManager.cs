...
namespace PluginMaster
{
    ...
    public static class SelectionManager
    {
        ...
        public static void ToggleSelection(GameObject obj)
        {
            var selectionList = UnityEditor.Selection.objects.ToList();
            if (selectionList.Contains(obj))
            {
                if (selectionList.Remove(obj))
                {
                    UnityEditor.Selection.objects = selectionList.ToArray();
                    UpdateSelection();
                }
            }
            else
            {
                UnityEditor.Selection.objects = selectionList.Append(obj).ToArray();
                UpdateSelection();
            }
        }
    }
